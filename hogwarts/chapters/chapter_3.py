...
def learn_spells(character):
    ...
    while learn[0] < 3 or learn[1] < 1 or learn[2] < 1:
        ...
        if spells[spell]['type'] == 'Utility' and learn[0] < 3:
            print(f"You have learned the spell {spells[spell]['name']} ({spells[spell]['type']}) !")
            character['Spells'].append(spells[spell])
            learn[0] += 1
            a = input("Press Enter to continue...")
        if spells[spell]['type'] == 'Offensive' and learn[1] < 1:
            print(f"You have learned the spell {spells[spell]['name']} ({spells[spell]['type']}) !")
            character['Spells'].append(spells[spell])
            learn[1] += 1
            a = input("Press Enter to continue...")
        if spells[spell]['type'] == 'Defensive' and learn[2] < 1:
            print(f"You have learned the spell {spells[spell]['name']} ({spells[spell]['type']})!")
            character['Spells'].append(spells[spell])
            learn[2] += 1
            a = input("Press Enter to continue...")
    ...
    for i in range(len(character['Spells'])):
        print(f"- {character['Spells'][i]} ({spells[spell]['type']}) : {spells[spell]['description']}")
